...
\documentclass[12pt,a4paper]{article}  % Use this line if this document is a draft
\usepackage{ifdraft}
...
%\begin{proposition}
%    \label{prop:sumint}
%    Suppose that~$f$ is a measurable function on $\RR$ with~$\|f\|_\infty <1$. Then
%    \begin{equation*}
...
%    \end{equation*}
%    Again, due to~$\|f\|_\infty<1$, we can integrate the above series term by term. The desired
...
%Applying this proposition to a piecewise constant function with finite pieces, we obtain the following result.
    ...
    Suppose that~$a_1$, ..., $a_n$ are located in $(-1, 1)$. Prove that
    \begin{equation*}
        \prod_{1\le i, j\le n} \frac{1+a_ia_j}{1-a_ia_j} \;\ge\;1.
    \end{equation*}

    Find the condition for the equality to hold.
